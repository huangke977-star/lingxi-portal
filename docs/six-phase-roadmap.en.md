...
| Phase 18 | 2026-09-04 | `0e2a876`, `0be84fa` | Deployed; Actions `33832810024` succeeded; P18 migration applied | Explainable recommendations and feedback recovery, delivery records, download/failure retry, idempotent refunds, point top-ups/violation penalties, article/resource-block creator earnings aggregates, and administrator boundaries are complete. 46 API suites and 325 tests passed; API/Web lint, production builds, Prisma validation, and production health checks passed; stopped containers and Lingxi dangling images were removed |
...
- Prisma generate/validate, API/Web lint, and production builds passed. The full API suite passed with 43 suites and 278 tests.
...
- Prisma migration `20260904100000_add_p18_recommendations_and_resources` is applied in production. The full API suite passed with 46 suites and 325 tests; API/Web lint, production builds, Prisma validation, and `git diff --check` passed. Actions `33832810024` built both API/Web images successfully; local and public health endpoints returned `status: ok`, and the home page returned HTTP 200. A database backup was created and gzip-verified before the final release.
