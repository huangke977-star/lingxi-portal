...
- **Dependency assessment**: the page reads API/Web `package.json` files and production Compose image versions, marking pinned, ranged, or unreadable entries with upgrade notes. The production API image carries the read-only metadata needed for this assessment; unreadable means the file is not visible in the current runtime, not that the dependency is unusable. It is an assessment, not an automatic production upgrade.
...
- **In-page load test**: System overview exposes only server-side read-only allowlisted endpoints. The API runs the job asynchronously and records the result under Recent runs; the browser never receives Docker access or an arbitrary URL target.
...
7. Alternatively open **Admin -> System overview -> P21 operational resilience -> Read-only load test**, choose endpoints, concurrency, and duration, then click **Start load test**. When it completes, open **Details** under **Recent runs** to inspect metrics and sample errors.
8. Before dependency upgrades, back up database and media and retain current API/Web image tags. Roll back images on failure; apply only forward-compatible migrations.
