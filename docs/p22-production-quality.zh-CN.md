...
- 超级管理员打开“后台管理 -> 系统运行概览”，点击“P22 生产质量 -> 一键验收”。按钮会同时执行服务端质量检查和当前浏览器中的公开页面验收，不需要记忆命令。
- 检查结果会记录在“最近演练记录”对应的“质量检查”中，点击“详情”查看具体项、指标和是否因 OSS/R2 待配置而阻塞。
- 当前浏览器验收结果会立即显示在 P22 区域，包括 9 项发布契约和桌面/移动端公开页面检查；刷新页面后仍可从运行记录查看服务端检查结果。
...
日常部署验收优先使用后台的“一键验收”。它不创建文章、评论、聊天或其他业务数据；页面检查在当前浏览器中运行，服务端检查由 API 执行。
...
视觉基线比较仍只建议在开发机执行。它需要读取和写入截图文件，适合页面设计确认，不属于每次部署都要执行的检查：
