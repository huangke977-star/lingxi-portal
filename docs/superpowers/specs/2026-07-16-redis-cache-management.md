...
- Cursor-based `SCAN` queries by literal key search and Redis type, limited to 100 keys per batch.
...
- Business cache TTL values can be updated; authentication cache TTL values cannot.
...
- Authentication TTL updates return 400 while business cache TTL updates succeed.
