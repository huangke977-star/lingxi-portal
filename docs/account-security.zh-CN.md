...
| `known_login_devices` | 用户已出现过的设备指纹、设备名称和首末 IP | 指纹优先使用 Web 端持久化的 `X-Device-ID`，否则使用 User-Agent 哈希 |
| `login_security_events` | 登录、风险、密码和邮箱验证事件 | 保存风险级别、IP、User-Agent、设备标签和必要元数据 |
| `email_verification_requests` | 注册和账号邮箱验证码请求 | 只保存 HMAC 哈希，不保存验证码明文 |
...
Redis 中的限流键是临时状态。清空 Redis 会同时清除限流计数和 Refresh Session，因此不能把清空 Redis 当作普通维护操作。
...
- `POST /auth/login`：登录；达到失败阈值后要求 Turnstile。
- `POST /auth/password-recovery/request`：申请密码找回邮件。
- `POST /auth/password-recovery/reset`：使用一次性令牌设置新密码。

### 登录用户接口

- `GET /auth/me/security`：读取邮箱状态、用户偏好、最近 30 条安全事件和最近 20 个已知设备。
...
设备指纹是稳定设备标识或 User-Agent 的 SHA-256 哈希，只能用于风险提示，不能证明设备真实身份。IP 来自可信反向代理传入的 `X-Forwarded-For` 首项；反向代理必须覆盖客户端伪造的该请求头。
...
默认配置下 SMTP、注册邮箱验证、密码找回和三个 Turnstile 开关均关闭，因此部署迁移本身不会把现有注册或登录流程绑定到外部服务。
...
9. 配置 Turnstile，按注册/找回、登录的顺序逐项开启和验证。
10. 观察邮件失败、HTTP `428`/`429`/`503`、登录风险事件和容器资源至少一个完整业务周期。
...
2. SMTP 故障时关闭注册邮箱验证和密码找回，再关闭 SMTP；避免注册页面要求无法送达的验证码。
...
6. 回滚后验证原有登录、注册、Refresh Token 刷新、管理员权限和健康接口。
...
9. 清理浏览器站点设备标识或换一个浏览器登录，确认生成 `new_device`；从新 IP 登录时确认生成相应风险记录。
10. 检查个人中心的邮箱状态、提醒开关、安全事件和设备列表；关闭新设备提醒后再次制造新设备事件，确认事件仍记录但不发送相应提醒。
11. 分别以管理员和超级管理员测试后台：两者都能查询邮件、验证码和风险事件，只有超级管理员能修改配置和测试 SMTP。
12. 临时使用错误 SMTP 地址制造发送失败，确认邮件任务记录错误；登录风险邮件失败时确认登录本身仍成功。
13. 检查 API 响应、容器日志、浏览器网络记录和 Git 历史，确认没有 SMTP 密码、Turnstile Secret、验证码、重置令牌或 `BACKUP_ENCRYPTION_KEY` 明文。
14. 生产验证结束后恢复正确配置，删除测试账号和不再需要的测试邮件记录时遵循审计保留要求。
