...
export const ANDROID_RELEASE_MAX_FILE_SIZE_BYTES = 120 * 1024 * 1024;

export interface UploadedAndroidPackage {
  fieldname: string;
  originalname: string;
  encoding: string;
  mimetype: string;
...
export function androidReleaseUploadDirectory(): string {
  return resolve(process.env.ANDROID_RELEASE_UPLOAD_DIR ?? join(process.cwd(), 'uploads', 'android-releases'));
}

export function createAndroidReleaseStorage() {
  const temporaryDirectory = join(androidReleaseUploadDirectory(), '.tmp');
    ...
    _handleFile(_request: unknown, file: IncomingFile, callback: StorageCallback): void {
      void (async () => {
        ...
        try {
          ...
          callback(error instanceof Error ? error : new Error('Android package upload failed.'));
        }
      })();
    },
