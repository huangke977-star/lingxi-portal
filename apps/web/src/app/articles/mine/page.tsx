        ...
        <aside className={`article-mine-sidebar${isCreatorDashboardOpen ? " is-open" : ""}`}>
          ...
          <section aria-label={phrase("创作者数据", "Creator dashboard")} className="article-creator-dashboard">
            ...
            <div className="article-resource-income">
              <header><span><Coins aria-hidden="true" size={15} /><strong>{phrase("资源收益明细", "Resource income")}</strong></span><Link href={localizedPath("/articles/resources/earnings", locale)}>{phrase("完整收益", "Full earnings")}</Link></header>
              {dashboard.recentResourceIncome.length ? <div>{dashboard.recentResourceIncome.map((income) => <Link href={localizedPath(`/articles/${income.article.slug}`, locale)} key={income.id}><span><strong>{income.article.title}</strong><small>{phrase(`兑换于 ${formatArticleDate(income.createdAt, locale)}`, `Unlocked ${formatArticleDate(income.createdAt, locale)}`)}{income.buyer ? ` · ${income.buyer.nickname}` : ""}</small></span><span><b><Coins aria-hidden="true" size={13} />{income.pointCost}</b><small className={income.deliveryStatus === "refunded" ? "refunded" : income.settledAt ? "settled" : "pending"}>{income.deliveryStatus === "refunded" ? phrase("已退款", "Refunded") : income.settledAt ? phrase(`已于 ${formatArticleDate(income.settledAt, locale)}到账`, `Settled ${formatArticleDate(income.settledAt, locale)}`) : phrase(`预计 ${formatArticleDate(income.availableAt, locale)}到账`, `Expected ${formatArticleDate(income.availableAt, locale)}`)}</small></span></Link>)}</div> : <p>{phrase("资源兑换后会在这里显示作者收益和到账状态。", "Resource unlocks will show their income and settlement status here.")}</p>}
            </div>
