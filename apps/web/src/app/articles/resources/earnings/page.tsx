...
export default function ResourceEarningsPage() {
  ...
  return <section className="page-shell articles-page resource-earnings-page">
    ...
    <header className="profile-subpage-heading"><div><span className="section-label"><History aria-hidden="true" size={14} /> RESOURCE EARNINGS</span><h1>{phrase("资源收益", "Resource earnings")}</h1><p>{phrase("查看文章资源按区块汇总的待入账、到账和退款状态。", "Review pending, settled, and refunded earnings by article resource block.")}</p></div><Link className="text-action" href={localizedPath("/articles/resources", locale)}>{phrase("返回资源中心", "Back to resources")}</Link></header>
