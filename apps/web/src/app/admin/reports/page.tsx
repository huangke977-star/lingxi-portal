...
function ModerationReportRow({ report, busy, onAction, onPreview }: { report: ModerationReport; busy: boolean; onAction: (report: ModerationReport, status: ModerationActionStatus) => void; onPreview: (report: ModerationReport) => void }) {
  ...
  return <article className={`moderation-report-row ${report.status}`} aria-label={`查看${report.sourceLabel}内容`} onClick={() => onPreview(report)} onKeyDown={handleKeyDown} role="button" tabIndex={0}>
    <div className="moderation-report-row-main"><div className="moderation-report-row-heading"><span className={`moderation-report-source ${report.source}`}><SourceIcon source={report.source} />{report.sourceLabel}</span><span className="moderation-report-reason">{REASON_LABEL[report.reason] ?? report.reason}</span><span className="moderation-report-heading-meta">{targetLabel}：{report.targetUser?.nickname || "未知用户"}{report.group ? ` · ${report.group.name}` : ""} · 举报人：{report.reporter.nickname}</span></div><div className="moderation-report-content"><strong>{content}</strong></div></div>
