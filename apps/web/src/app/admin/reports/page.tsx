...
const PAGE_SIZE = 20;
const STATUS_LABEL: Record<ModerationReportStatus | "all", string> = {
  pending: "待处理",
  resolved: "已处理",
  rejected: "已驳回",
  all: "全部状态",
};
const SOURCE_LABEL: Record<ModerationReportSource | "all", string> = {
  article: "文章举报",
  comment: "评论举报",
  group_message: "群消息举报",
  all: "全部来源",
...
export default function ModerationReportsPage() {
  ...
  useEffect(() => {
    ...
    let active = true;
    Promise.all([getMe(token), getModerationReportSummary(token)])
      .then(([currentUser, summaryResult]) => {
        if (!active) return;
      ...
      .catch((loadError) => {
        if (active) setError(loadError instanceof Error ? loadError.message : "举报中心读取失败。");
      });
    return () => { active = false; };
    // The initial protected workspace load should only run once; filters reload below.
  ...
  useEffect(() => {
    ...
    // Filter changes intentionally restart at the first page.
  ...
  async function submitAction() {
    ...
    try {
      if (report.source === "article") {
        await moderateArticleReport(token, report.id, { status: nextStatus, resolution: feedback });
      } else if (report.source === "comment") {
        await moderateCommentReport(token, report.id, { status: nextStatus, resolution: feedback });
      } else {
        await handleChatGroupReport(token, report.id, { status: nextStatus, resolution: feedback, deleteMessage: nextStatus === "resolved" });
      }
  ...
  function openAction(report: ModerationReport, nextStatus: ModerationActionStatus) {
    setActionTarget({ report, status: nextStatus });
    setResolution("");
  }
  ...
  return <section className="page-shell moderation-reports-page">
    <header className="moderation-reports-header">
      <div><span className="page-kicker">CONTENT MODERATION</span><h1>举报中心</h1><p>将文章、评论和群消息举报集中到同一条处理队列中。</p></div>
      <div className="moderation-summary"><span><b>{summary.pending}</b><small>待处理</small></span><span><b>{summary.total}</b><small>全部记录</small></span></div>
    </header>
    <div className="moderation-reports-toolbar">
      <div className="moderation-filter-group"><GlassSelect ariaLabel="举报状态" onChange={setStatus} options={Object.entries(STATUS_LABEL).map(([value, label]) => ({ value: value as ModerationReportStatus | "all", label }))} value={status} /><GlassSelect ariaLabel="举报来源" onChange={setSource} options={Object.entries(SOURCE_LABEL).map(([value, label]) => ({ value: value as ModerationReportSource | "all", label }))} value={source} /></div>
      <div className="moderation-source-summary"><span><FileText aria-hidden="true" size={14} />文章 {summary.bySource.article}</span><span><MessageSquare aria-hidden="true" size={14} />评论 {summary.bySource.comment}</span><span><Flag aria-hidden="true" size={14} />群消息 {summary.bySource.group_message}</span></div>
    </div>
    {isLoading ? <div className="article-empty-state"><LoaderCircle className="spin" size={22} />正在读取举报队列。</div> : items.length ? <div className="moderation-report-list">{items.map((report) => <ModerationReportRow busy={busyKey === report.key} key={report.key} onAction={openAction} report={report} />)}</div> : <div className="article-empty-state"><AlertTriangle size={22} />当前筛选下没有举报记录。</div>}
...
function ModerationReportRow({ report, busy, onAction }: { report: ModerationReport; busy: boolean; onAction: (report: ModerationReport, status: ModerationActionStatus) => void }) {
  ...
  return <article className={`moderation-report-row ${report.status}`}>
    <div className="moderation-report-row-main">
      <div className="moderation-report-row-heading"><span className={`moderation-report-source ${report.source}`}><SourceIcon source={report.source} />{report.sourceLabel}</span><span className="moderation-report-reason">{REASON_LABEL[report.reason] ?? report.reason}</span><time>{formatTime(report.createdAt)}</time></div>
      <div className="moderation-report-content"><strong>{content}</strong>{report.detail ? <p>{report.detail}</p> : null}</div>
      <div className="moderation-report-meta"><span><UserRound size={14} />{targetLabel}：{report.targetUser?.nickname || "未知用户"}</span>{report.group ? <span>群聊：{report.group.name}</span> : null}<span>举报人：{report.reporter.nickname}</span></div>
    </div>
    <footer className="moderation-report-row-actions">
      {report.article ? <Link aria-label="打开文章管理" href={`/admin/articles?report=${report.id}&reportSource=${report.source}`} title="打开文章管理"><ExternalLink size={15} /></Link> : null}
      {report.group ? <Link aria-label="打开群聊举报" href="/admin/groups?tab=reports" title="打开群聊举报"><ExternalLink size={15} /></Link> : null}
      {report.status === "pending" ? <><button aria-label="驳回举报" disabled={busy} onClick={() => onAction(report, "rejected")} title="驳回举报" type="button"><X size={16} /></button><button aria-label="处理举报" className="confirm" disabled={busy} onClick={() => onAction(report, "resolved")} title={report.source === "group_message" ? "处理并删除群消息" : "处理举报"} type="button">{busy ? <LoaderCircle className="spin" size={16} /> : <Check size={16} />}</button></> : <small>{report.status === "resolved" ? "已处理" : "已驳回"}</small>}
    </footer>
  </article>;
}

function SourceIcon({ source }: { source: ModerationReportSource }) {
  if (source === "article") return <FileText aria-hidden="true" size={14} />;
  if (source === "comment") return <MessageSquare aria-hidden="true" size={14} />;
  return <Flag aria-hidden="true" size={14} />;
}
