...
export default function SystemStatusPage() {
  ...
  async function handleQualityCheck() {
    ...
    try {
      ...
      setError(actionError instanceof Error ? actionError.message : phrase("质量检查失败。", "Quality check failed."));
    } finally {
      ...
      <div className="system-status-grid">
        ...
        {operations ? <section className="system-status-panel p21-operations-panel">
          <header className="system-panel-heading p21-heading">
            <span><ClipboardCheck aria-hidden="true" size={17} /><strong>{phrase("运营韧性", "Operational resilience")}</strong></span>
          ...
          <section className="p21-subpanel p21-load-test-panel"><header><strong>{phrase("只读压测", "Read-only load test")}</strong><small>{phrase("仅能选择白名单接口；单次最多 50 并发、300 秒。", "Only allowlisted endpoints are available; one run is limited to 50 workers and 300 seconds.")}</small></header><div className="p21-load-test-form">
            ...
            <label><span>{phrase("并发数", "Concurrency")}</span><span className="p21-number-field"><input disabled={Boolean(operationsBusy)} max={50} min={1} onChange={(event) => setLoadTestForm((current) => ({ ...current, concurrency: Number(event.target.value) }))} type="number" value={loadTestForm.concurrency} /><em>{phrase("并发", "workers")}</em></span></label>
        ...
        {quality ? <section className="system-status-panel p22-quality-panel">
          ...
          {quality.latestRun ? <p className="p22-quality-last">{phrase("最近检查", "Latest check")}：{runStatusLabel(quality.latestRun.status, phrase)} · {formatDateTime(quality.latestRun.createdAt, locale)} · {quality.latestRun.summary}</p> : <p className="p22-quality-last">{phrase("尚未执行质量检查。", "Quality check has not run yet.")}</p>}
        </section> : null}
