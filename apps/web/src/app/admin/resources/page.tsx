...
export default function AdminResourcesPage() {
  ...
  async function adjust(kind: "top-up" | "violation") {
    const token = readAccessToken(); const parsedUserId = Number(userId); const parsedPoints = Number(points);
    if (!token || !Number.isInteger(parsedUserId) || parsedUserId < 1 || !Number.isInteger(parsedPoints) || parsedPoints < 1 || !eventKey.trim()) { setError(phrase("请填写有效的用户 ID、积分数量和唯一事件键。", "Enter a valid user ID, point amount, and unique event key.")); return; }
    try { const result = kind === "top-up" ? await topUpUserPoints(token, { userId: parsedUserId, points: parsedPoints, eventKey: eventKey.trim(), note }) : await applyViolationPenalty(token, { userId: parsedUserId, points: parsedPoints, eventKey: eventKey.trim(), note }); setNotice(result.applied ? phrase("积分账本已更新。", "Points ledger updated.") : phrase("该事件键已经处理过。", "This event key was already processed.")); setPoints(""); setEventKey(""); setNote(""); }
