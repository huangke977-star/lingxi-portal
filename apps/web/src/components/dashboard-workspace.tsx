  ...
  return <section className="p8-page p8-dashboard-page">
    ...
    {isLoading ? <div className="status-row compact-status-row"><span className="status">正在读取工作台</span></div> : user ? <>
      ...
      <div className="p8-dashboard-grid">
        ...
        {canManage ? <section className="p8-surface p8-dashboard-card management"><div className="p8-dashboard-card-heading"><ShieldAlert aria-hidden="true" size={18} /><span><h2>管理待办</h2><small>举报与匿名话题管理</small></span><b>{moderationCount}</b></div><div className="p8-dashboard-links"><Link href="/admin/reports">统一举报中心 {moderationCount}</Link><Link href="/admin/voices"><MessagesSquare aria-hidden="true" size={13} />匿名话题</Link></div><Link className="p8-card-footer-link" href="/admin/reports">进入举报中心</Link></section> : <SuggestionsPanel className="p8-dashboard-suggestion-card" mode="mine" pageSize={8} title="我的建议" />}
      </div>
