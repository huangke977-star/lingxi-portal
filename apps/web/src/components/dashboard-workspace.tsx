...
export function DashboardWorkspace() {
  ...
  return <section className="p8-page p8-dashboard-page">
    <header className="p8-page-heading"><div><span className="section-label">{t("dashboard.section")}</span><h1>{t("dashboard.title")}</h1></div><Link className="p8-primary-link" href={localizedPath("/articles/write", locale)}><FilePenLine aria-hidden="true" size={16} />{t("dashboard.writeArticle")}</Link></header>
    {isLoading ? <div className="status-row compact-status-row"><span className="status">{t("dashboard.loading")}</span></div> : user ? <>
      ...
      <div className="p8-dashboard-grid">
        <section className="p8-surface p8-dashboard-card"><div className="p8-dashboard-card-heading"><FilePenLine aria-hidden="true" size={18} /><span><h2>{t("dashboard.myCreation")}</h2><small>{t("dashboard.articleStatus")}</small></span><b>{data.article.total}</b></div><div className="p8-dashboard-links"><Link href="/articles/mine">{t("dashboard.drafts")} {data.article.draft}</Link><Link href="/articles/mine">{t("dashboard.published")} {data.article.published}</Link><Link href="/articles/mine">{t("dashboard.unpublished")} {data.article.unpublished}</Link></div><Link className="p8-card-footer-link" href="/articles/mine">{t("dashboard.enterCreation")}</Link></section>
        ...
        {canManage ? <section className="p8-surface p8-dashboard-card management"><div className="p8-dashboard-card-heading"><ShieldAlert aria-hidden="true" size={18} /><span><h2>{t("dashboard.managementTodo")}</h2><small>{t("dashboard.managementNote")}</small></span><b>{moderationCount}</b></div><div className="p8-dashboard-links"><Link href="/admin/reports">{t("dashboard.reportCenter")} {moderationCount}</Link><Link href="/admin/voices"><MessagesSquare aria-hidden="true" size={13} />{t("dashboard.anonymousTopics")}</Link></div><Link className="p8-card-footer-link" href="/admin/reports">{t("dashboard.enterReportCenter")}</Link></section> : <SuggestionsPanel className="p8-dashboard-suggestion-card" mode="mine" pageSize={8} title={t("dashboard.mySuggestions")} />}
      </div>
