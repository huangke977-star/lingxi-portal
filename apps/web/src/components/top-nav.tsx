          ...
          {user && roleBadge ? <>
            ...
            <div className="account-menu-wrap" ref={accountMenuRef} onPointerEnter={(event) => handleHoverOpen(event, accountMenuCloseTimerRef, () => setIsAccountMenuOpen(true))} onPointerLeave={(event) => { if (event.pointerType === "mouse") scheduleClose(accountMenuCloseTimerRef, () => setIsAccountMenuOpen(false)); }}>
              ...
              <div className={`account-menu ${isAccountMenuOpen ? "open" : ""}`} onFocus={() => cancelClose(accountMenuCloseTimerRef)} role="menu">
                ...
                {user.isSuperAdmin || user.role.level >= 90 ? <><Link href="/admin" onClick={() => setIsAccountMenuOpen(false)}>用户管理</Link><Link href="/admin/content" onClick={() => setIsAccountMenuOpen(false)}>内容管理</Link><Link href="/admin/articles" onClick={() => setIsAccountMenuOpen(false)}>文章举报</Link><Link href="/admin/violations" onClick={() => setIsAccountMenuOpen(false)}>违规作者</Link><Link href="/admin/feedback" onClick={() => setIsAccountMenuOpen(false)}>用户反馈管理</Link><Link href="/admin/groups" onClick={() => setIsAccountMenuOpen(false)}>群聊管理</Link><Link href="/admin/voices" onClick={() => setIsAccountMenuOpen(false)}>匿名话题管理</Link><Link href="/admin/analytics" onClick={() => setIsAccountMenuOpen(false)}>运营数据</Link><Link href="/admin/announcements" onClick={() => setIsAccountMenuOpen(false)}>公告管理</Link><Link href="/admin/audit" onClick={() => setIsAccountMenuOpen(false)}>审计日志</Link><Link href="/admin/security" onClick={() => setIsAccountMenuOpen(false)}><ShieldCheck aria-hidden="true" size={15} />安全管理</Link></> : null}
