          ...
          <div className="chat-dock-titlebar-actions">
            ...
            {selected && !selectedNotificationConfig ? messageSearchOpen ? <div className="chat-inline-message-search" onPointerDown={(event) => event.stopPropagation()} ref={messageSearchRef}>
              ...
              <div className="chat-message-search-dropdown">
                ...
                {messageSearchResults.map((result) => <button className="chat-message-search-result" key={`${result.conversation.id}-${result.id}`} onClick={() => void openMessageSearchResult(result)} type="button"><span><strong>{result.body || phrase("附件消息", "Attachment")}</strong><small>{result.senderDisplayName} · {formatChatTime(result.createdAt, locale)}</small></span></button>)}
              </div>
