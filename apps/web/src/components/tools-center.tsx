...
function ToolLink({ record }: { record: ToolRecord }) {
  const content = <><PortalEntryVisual entry={record.entry} /><span><strong>{record.entry.title}</strong><small>{record.entry.description || portalHost(record.entry.url)}</small></span></>;
