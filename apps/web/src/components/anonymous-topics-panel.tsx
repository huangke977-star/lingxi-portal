...
import { EyeOff, KeyRound, LoaderCircle, Lock, MessageCircleMore, Plus, Search, Send, ThumbsDown, ThumbsUp, Unlock, UserRoundPen, X } from "lucide-react";
...
export function AnonymousTopicsPanel({ pageSize = 5, title = "匿名话题", moreHref, showLoadMore = false, showSearch = false }: { pageSize?: number; title?: string; moreHref?: string; showLoadMore?: boolean; showSearch?: boolean }) {
  ...
  const loadTopics = useCallback(async (pageNumber = 1, append = false) => {
    setIsLoading(true);
    try {
      const page = await listAnonymousTopics(pageNumber, pageSize, query);
  ...
  return <section className="p8-surface anonymous-topics-panel">
    ...
    {showSearch ? <label className="p8-list-search"><Search aria-hidden="true" size={16} /><input onChange={(event) => setQuery(event.target.value)} placeholder="搜索话题" value={query} /></label> : null}
