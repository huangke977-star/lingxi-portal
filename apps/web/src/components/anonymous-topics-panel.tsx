...
import { Eye, EyeOff, Heart, KeyRound, LoaderCircle, Lock, MessageCircleMore, Plus, Search, ThumbsDown, ThumbsUp, Unlock, UserRoundPen, X } from "lucide-react";
...
export function AnonymousTopicsPanel({ initialSort = "time", management = false, pageSize = 5, title, moreHref, showLoadMore = false, showSearch = false, showSort = false }: { initialSort?: AnonymousTopicSort; management?: boolean; pageSize?: number; title?: string; moreHref?: string; showLoadMore?: boolean; showSearch?: boolean; showSort?: boolean }) {
  ...
  async function sendMessage(files: File[]): Promise<boolean> {
    if (!topic || (!message.trim() && !files.length)) return false;
    setIsSaving(true);
    try {
      const identity = readAnonymousIdentity(topic.id);
      const sent = await sendAnonymousMessage(topic.id, message, identity?.identityToken, files);
      ...
      await loadTopics();
      return true;
    } catch (saveError) { setError(saveError instanceof Error ? saveError.message : phrase("发送失败。", "Could not send the message.")); return false; }
    finally { setIsSaving(false); }
  }
