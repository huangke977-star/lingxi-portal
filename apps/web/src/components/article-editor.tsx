...
import { CalendarClock, Check, ChevronDown, CircleHelp, Cloud, CloudOff, Eye, FilePlus2, History, RefreshCw, RotateCcw, Save, Send, Shapes, Sparkles, Tags, Trash2, X } from "lucide-react";
  ...
  return (
    <section className="page-shell articles-page article-editor-page">
      ...
      {isPreviewOpen && typeof document !== "undefined" ? createPortal(
        <div className="article-preview-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) setIsPreviewOpen(false); }}>
          <section aria-label={phrase("文章发布预览", "Article publish preview")} aria-modal="true" className="article-publish-preview" role="dialog">
            <header className="article-preview-toolbar">
              <span><Eye aria-hidden="true" size={17} /><strong>{phrase("发布预览", "Publish preview")}</strong></span>
              <button aria-label={phrase("关闭预览", "Close preview")} onClick={() => setIsPreviewOpen(false)} title={phrase("关闭", "Close")} type="button"><X aria-hidden="true" size={18} /></button>
            </header>
            <article className="article-reading-layout article-publish-preview-layout">
              ...
              <div className="article-reading-grid preview article-publish-preview-grid">
                <main className="article-reading-main"><ArticleBody content={draft.content || phrase("开始输入后，这里会显示文章预览。", "Start writing to see the article preview here.")} contentFormat={draft.contentFormat} pendingImageUrls={pendingImageUrls} /></main>
              </div>
            </article>
          </section>
        </div>,
      ...
      {isAiDialogOpen && typeof document !== "undefined" ? createPortal(
        <div className="modal-backdrop article-ai-dialog-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) setIsAiDialogOpen(false); }}>
          <section aria-label={phrase("AI 文章助手", "AI writing assistant")} aria-modal="true" className="announcement-editor article-ai-dialog" role="dialog">
            <header><span><Sparkles aria-hidden="true" size={17} /><strong>{phrase("AI 文章助手", "AI writing assistant")}</strong><button aria-label={phrase("AI 助手说明", "AI assistant help")} className="article-ai-help" data-tooltip={phrase("选择一个操作，生成结果后先预览；只有点击应用才会写入文章。", "Choose an action and preview its result first. Nothing is written to the article until you apply it.")} title={phrase("选择一个操作，生成结果后先预览；只有点击应用才会写入文章。", "Choose an action and preview its result first. Nothing is written to the article until you apply it.")} type="button"><CircleHelp aria-hidden="true" size={16} /></button></span><button aria-label={phrase("关闭", "Close")} onClick={() => setIsAiDialogOpen(false)} title={phrase("关闭", "Close")} type="button"><X size={17} /></button></header>
            <div className="announcement-editor-body article-ai-dialog-body"><label className="article-ai-operation"><span>{phrase("助手操作", "Assistant action")}</span><GlassSelect ariaLabel={phrase("助手操作", "Assistant action")} leadingIcon={<Sparkles size={14} />} onChange={(value) => { setAiOperation(value as ArticleAssistantOperation); setAiResult(null); }} options={ARTICLE_ASSISTANT_OPERATIONS.map((item) => ({ value: item.value, label: phrase(item.label, item.english) }))} value={aiOperation} /></label>{aiResult ? <div className="article-ai-result"><div className="article-ai-result-meta"><span>{phrase("生成预览", "Generated preview")}</span><small>{aiResult.model} · {aiResult.durationMs} ms</small></div>{aiResult.operation === "title" || aiResult.operation === "summary" || aiResult.operation === "taxonomy" ? <pre>{aiResult.text}</pre> : <ArticleBody content={aiResult.text} contentFormat="markdown" />}</div> : <p className="article-ai-empty">{aiSelectedText ? phrase("已选择一段正文；正文类操作会优先处理该段内容。", "A passage is selected. Body actions will prioritize that selection.") : phrase("当前文章标题和正文会作为上下文发送给已配置的 AI。", "The current article title and body will be sent as context to the configured AI.")}</p>}</div>
            <footer><button aria-label={phrase("生成预览", "Generate preview")} className="article-template-icon-button primary" disabled={isAiLoading} onClick={() => void generateAiSuggestion()} title={phrase("生成预览", "Generate preview")} type="button"><Sparkles className={isAiLoading ? "spin" : undefined} size={17} /></button>{aiResult ? <button aria-label={phrase("应用结果", "Apply result")} className="article-template-icon-button primary" onClick={applyAiSuggestion} title={phrase("应用结果", "Apply result")} type="button"><Check size={17} /></button> : null}</footer>
          </section>
        </div>, document.body,
      ) : null}
